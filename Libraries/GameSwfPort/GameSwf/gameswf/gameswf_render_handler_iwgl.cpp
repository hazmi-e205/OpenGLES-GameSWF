...
struct render_handler_iwgl : public gameswf::render_handler
{
	// Some renderer state.
	...
	if (m_texture_id == 0)
	{
		...
		switch (m_suspended_image->m_type)
		{
			...
			case image::image_base::ALPHA:
			{
				...
				while (w > 1 || h > 1)
				{
					render_handler_iwgl::make_next_miplevel(&w, &h, m_suspended_image->m_data);
...
namespace gameswf
{
render_handler*	create_render_handler_iwgl()
// Factory.
{
	return new render_handler_iwgl();
}
}
